...
import type { DBQueryConfig, Operators, OrderByOperators, TableRelationalConfig, TableRelationalConfig, TablesRelationalConfig } from "drizzle-orm";
...
type TDBQueryConfig<T extends TableRelationalConfig = TableRelationalConfig> = DBQueryConfig<"many", true, TablesRelationalConfig, T>;

/**
 * Create a order by clause for SQL query
 */
export const orderByQueryBuilder =
  <T extends TableRelationalConfig = TableRelationalConfig>(_orderBy: z.infer<typeof orderBySchema> = [], defaultColumn: keyof T["columns"] = "createdAt"): TDBQueryConfig<T>["orderBy"] =>
    (fields, operators) =>
      _orderBy.length > 0 ? _orderBy.map((item) => (item.desc ? operators.desc(fields[item.id]) : operators.asc(fields[item.id]))) : [operators.asc(fields?.[defaultColumn])];
